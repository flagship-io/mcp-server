...
function generateCodebaseAnalyzerPrompts(): string {
  const promptsDir = join(__dirname, "../assistant-prompts/codebase-analyzer");
  const files = readdirSync(promptsDir).filter((file) => file.endsWith(".md"));
...
try {
  ...
  // Codebase Analyzer Prompts
  const codebaseAnalyzer = generateCodebaseAnalyzerPrompts();
  const codebaseAnalyzerOutputPath = join(
    __dirname,
    "../src/prompts/generated/codebaseAnalyzer.generated.ts"
  );

  mkdirSync(dirname(codebaseAnalyzerOutputPath), { recursive: true });

  writeFileSync(codebaseAnalyzerOutputPath, codebaseAnalyzer, "utf-8");
  ...
  console.log(
    `✓ Generated codebase analyzer prompts written to: ${codebaseAnalyzerOutputPath}`
  );
